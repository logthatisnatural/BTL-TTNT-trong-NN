...
class LlamaEmbeddingClassifier(torch.nn.Module):
	...
	def forward(self, input_ids):
		...
		_, hidden_states = self.llama(input_ids) # hidden_states: (batch_size, seq_len, dim)
		
		# 1) Get the hidden state for the last token in each sequence
		# Using the last hidden state from the sequence
		last_hidden_state = hidden_states[:, -1, :] # Lấy hidden state của token cuối cùng: (batch_size, dim)
		
		# 2) Apply dropout to the hidden state
		dropped_hidden_state = self.dropout(last_hidden_state)
		
		# 3) Pass through the classifier head to get logits
		logits = self.classifier_head(dropped_hidden_state) # (batch_size, num_labels)
		
		# 4) Apply log_softmax to get log probabilities
		log_probabilities = F.log_softmax(logits, dim=-1) # (batch_size, num_labels)
