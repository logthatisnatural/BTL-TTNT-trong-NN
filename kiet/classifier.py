...
class LlamaEmbeddingClassifier(torch.nn.Module):
	...
	def forward(self, input_ids):
		...
		'''
		# todo
		# 1. Lấy hidden state từ Llama model
        # Hàm llama trả về (logits, hidden_states)
		...
		last_hidden_state = hidden_states[:, -1, :]  # (batch_size, hidden_dim)
			
		# 2. Áp dụng dropout trong quá trình training
		hidden_with_dropout = self.dropout(last_hidden_state)
			
		# 3. Đưa qua classifier head để có logits
		logits = self.classifier_head(hidden_with_dropout)  # (batch_size, num_labels)
			
		# 4. Áp dụng log-softmax để có log-probabilities
		log_probs = F.log_softmax(logits, dim=-1)  # (batch_size, num_labels)
