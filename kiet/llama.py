...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        '''
        # todo
        # (bs, n_local_heads, seqlen, head_dim) @ (bs, n_local_heads, head_dim, seqlen)
        ...
        # Apply softmax để có được attention probabilities
        attn_weights = F.softmax(attn_weights, dim=-1)
        
        # Apply dropout cho attention weights
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # 3. Residual connection từ input
        h = x + attention_output
        
        # 4. Layer norm trước feed-forward
        ffn_input = self.ffn_norm(h)
        
        # 5. Feed-forward network
        ffn_output = self.feed_forward(ffn_input)
        
        # 6. Residual connection từ output của attention
        output = h + ffn_output
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0):
        ...
        for _ in range(max_new_tokens):
            ...
            # Chọn index có xác suất cao nhất
                ...
                # 1. Scale logits bằng temperature
                scaled_logits = logits / temperature
                
                # 2. Chuyển thành probabilities qua softmax
                probs = F.softmax(scaled_logits, dim=-1)
                
                # 3. Sample từ phân phối xác suất
                idx_next = torch.multinomial(probs, num_samples=1)
            
            # Thêm index được chọn vào sequence
